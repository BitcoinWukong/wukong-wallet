...
class ZBarDecoder:
    ...
    def validate_code_types(self, code_types):
        ...
        if not all(code_type in available_code_types for code_type in code_types):
            raise ValueError(
...
class PyZBarDecoder(ZBarDecoder):
    ...
    def decode(self, image, code_types):
        ...
        pyzbar_code_types = set(
            getattr(self.pyzbar.ZBarSymbol, code_type) for code_type in code_types
        )
        return [
...
class ZBarLightDecoder(ZBarDecoder):
    ...
    def decode(self, image, code_types):
        self.validate_code_types(code_types)
        zbarlight_code_types = set(code_type.lower() for code_type in code_types)
        codes = self.zbarlight.scan_codes(zbarlight_code_types, image)

        # zbarlight.scan_codes() returns None instead of []
        if not codes:
            return []

        return [ZBarCam.Symbol(type=None, data=code) for code in codes]
...
class XZbarDecoder(ZBarDecoder):
    ...
    def __init__(self):
        ...
        XZbarDecoder.zbar_decoder = self.zbar_decoder or self._get_implementation()
